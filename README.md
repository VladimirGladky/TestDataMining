...
Lenta стоит за Qrator, поэтому сессия инициализируется один раз через
headless Chrome: он проходит челлендж, забирает cookies + `sessiontoken`
и передаёт обычному `http.Client`, который дальше собирает данные напрямую.
Никаких ручных копирований из DevTools.
...
## Где взять selectionId категории

ID видно прямо в URL категории на сайте. Зайти на lenta.com, кликнуть в
нужную категорию — например «Фрукты» — и посмотреть адресную строку:

```
https://lenta.com/catalog/frukty-310002385
                          └──────┘└──────┘
                            slug      id
```

Число после дефиса (`310002385`) и есть `selectionId`. Несколько категорий —
через запятую: `LENTA_SELECTION_IDS=310002385,310003460`.
